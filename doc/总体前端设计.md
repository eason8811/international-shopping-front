...
- 国际化：`next-intl`（locale 路由 + 消息文件驱动）。
...
1. **App Router 路由层**：页面路由、layout、loading/error、metadata、locale 分段路由。
...
- 适用范围: 所有本项目 `src/**/*.ts` 与 `src/**/*.tsx` 代码文件, 不包含 `json/css/svg` 等非代码资源文件。
- 变更要求: 每次新增或修改代码文件时, 需补齐完整中文 JsDoc, 至少覆盖导出的 `type/interface/function/component/const`。
- 内容要求: JsDoc 需说明职责, 入参, 返回值, 关键副作用, 以及与上下游模块的关系。
- 标点要求: 注释文本仅使用英文标点, 分号, 顿号等统一替换为英文逗号, 不使用中文句号。
- 排版要求: 英文括号与中文相邻时, 括号外侧各保留一个空格, 英文逗号后保留一个空格。
...
- `src/proxy.ts` 统一承载：
  - `next-intl` 的 locale 协商与 `/{locale}` 路由前缀处理。
  - 前端路由守卫（未登录访问 `/me/*`、`/admin/*` 跳转 `/{locale}/login`）。
- `matcher` 需排除 `/api`、`/_next`、静态资源与监控探针路径。
- 注意：前端守卫仅提升体验，不替代后端鉴权。

## 9. 迭代计划（细化版，建议）

### 9.1 里程碑节奏（建议 10-12 周）

- M0（第 1 周）：工程底座可用，BFF/鉴权/CSRF/错误模型/i18n 路由打通。
...
- 范围：
  ...
  - 建立 `next-intl` 国际化骨架：`routing.ts`、`request.ts`、`messages/*`、`proxy.ts`。
  - 页面路由切换到 `app/[locale]/*`，保证用户侧与管理侧可按语言扩展。
- 关键接口：
  - `GET /auth/csrf`、`POST /auth/refresh-token`、`GET /users/me`。
- 验收标准：
  - 90% 以上写接口通过统一 BFF 客户端发送。
  - 未登录、CSRF 失败、429、5xx 在 UI 上有统一且可追踪的错误展示。
  - 根路径 `/` 可按 locale 协商跳转，页面文案由消息文件驱动，不在页面硬编码多语言副本。
- 完成情况（截至 2026-03-06）：
  - 已完成 `app/bff/features/entities/shared` 的工程骨架与核心请求通道, 建立统一 `Result<T>` 解包与错误映射。
  - 已完成 Cookie 鉴权, CSRF 双提交, 幂等键注入能力, 并在 BFF 层统一收敛协议细节。
  - 已完成 `next-intl` 路由与消息文件底座, 页面路由已切换至 `app/[locale]/*`。
  - 已通过简单验收, 当前状态为已完成。
...
- 验收标准：
  ...
  - 地址写接口幂等重试不产生重复地址记录。
- 完成情况（截至 2026-03-06）：
  - 已完成登录, 注册, 找回密码, 邮箱状态查询与验证码激活流程, 并支持 `GOOGLE`、`TIKTOK`、`X` 三方 OAuth2 登录。
  - 已完成 `user` 域 `auth/*`, `/users/me`, `/users/me/profile`, `/users/me/addresses*` 的 BFF 映射与前端调用链路。
  - 已完成个人信息维护, 地址 CRUD, 默认地址设置, 地址创建幂等保护, 并对齐 PC/Pad/Mobile 三端一致风格与动效基线。
  - 已清理 Phase 0 用于测试验收的临时代码组件, 当前状态为已完成并通过构建验收（`pnpm lint`, `pnpm build`）。
...
- 用户可见文案默认走 `next-intl` 消息键，禁止在页面硬编码多语言文本（调试信息除外）。
...
- UX-M0（第 1 周）：设计基础冻结  
  ...
  参考产物：`doc/design-tokens-v1-freeze.md`、`doc/design-tokens-v1.tweakcn.css`（可直接导入 tweakcn 预览）。
  当前状态：`v1.0` 已于 `2026-03-02` 冻结并同步到 `src/app/globals.css`。
...
- next-intl App Router（含 locale routing）：https://next-intl.dev/docs/getting-started/app-router/with-i18n-routing
- next-intl App Router 快速接入：https://next-intl.dev/docs/getting-started/app-router
